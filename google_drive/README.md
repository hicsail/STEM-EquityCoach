...
2. _(Optional) Create a folder `local` to put in documents that you want to load from your local machine._

3. _(Optional) Create Python virtual environment `python -m venv .venv`._

4. _(Optional) Activate the virtual environment `source .venv/bin/activate`. You can deactivate it by running `deactivate`._

5. Install dependencies `pip install -r requirements.txt`.

6. _(Optional) Add persisted Chroma DB. Simply move your `chroma` folder under `/google_drive`._

7. Run the scripts `python main.py`. You can add an optional flag `-c` or `--context` with choice between `name` or `raw` to display context documents.
